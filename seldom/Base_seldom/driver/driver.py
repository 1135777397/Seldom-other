...
class ChromeConfig:
    """
    Firefox和Chrome浏览器支持headless模式，即将浏览器置于后台运行，这样不会影响到我们在测试机上完成其他工作。
    将要ChromeOption添加的设置赋值给ChromeConfig的options变量。
    指定浏览器驱动
    command_executor:输入驱动位置/设置远程节点
...
class Browser(object):
    ...
    to drive the Chrome browser. Of course, you can also
    ...
    def __new__(cls, name=None):
        ...
        if (cls.name is None) or (cls.name in ["chrome", "google chrome", "gc"]):
            ...
            return cls.pad(name)
        raise NameError(
            "Not found '{}' browser".format(cls.name))
    ...
    @staticmethod
    def ie():
        if IEConfig.command_executor != "" and IEConfig.command_executor[:4] == "http":
