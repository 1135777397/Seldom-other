...
class IndexPage(Page):
    """登录页面"""
    account = Element(xpath="//input[@placeholder='请输入帐号']")  # 用户名框
    password = Element(xpath='//input[@placeholder="请输入密码"]')  # 密码
    login_but = Element(xpath="//button[@type='button']")  # 登录按钮
    index_logout = Element(xpath="//span[text()='退出']")  # 退出按钮
    index_role_but = Element(xpath="//span[@role='button']")  # 带登录用户名的按钮
    index_logo = Element(xpath="//div[@class='header']//i")  # 登录页面logo
    index_label_frontpage = Element(xpath="//span[text()=' 首页 ']")  # 首页标签
    index_light = Element(xpath="//div[@class='header-icon-wrap'][1]")  # 指示灯按钮,希望后续加id，name
